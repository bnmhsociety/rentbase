import { supabase, mediaProxyUrl } from "./supabase";
...
export async function getAgencyBySlug(slug) {
  ...
  const profilePhotoUrl = row.website_profile_photo_path
    ? mediaProxyUrl("rentbase-photos", row.website_profile_photo_path)
    : null;
...
async function hydrateVehicle(vehicle) {
  ...
  for (const path of possiblePaths) {
    imageUrl = mediaProxyUrl("rentbase-photos", path);
    if (imageUrl) break;
  }
