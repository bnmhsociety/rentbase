...
export async function getAgencyBySlug(slug) {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .or(
      `website_slug.ilike.${cleanSlug},website_url.ilike.%/${cleanSlug}%`
    )
    .limit(1)
    .maybeSingle();

  if (error) {
    console.log("Erreur getAgencyBySlug:", error);
    throw error;
  }

  if (!data) {
    console.log("Agence introuvable pour slug:", cleanSlug);
    return null;
  }
  ...
  if (data.website_profile_photo_path) {
    profilePhotoUrl = await signedUrl(
      "rentbase-photos",
      data.website_profile_photo_path,
      3600
    );
  }
