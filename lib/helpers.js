...
export function fmtDateTime(date, hour = "") {
  if (!date) return "—";
  const value = hour ? `${String(date).slice(0, 10)}T${hour}:00` : date;
  const d = new Date(value);
  if (Number.isNaN(d.getTime())) return `${date || ""} ${hour || ""}`.trim() || "—";
...
export function toIso(date, hour = "09:00") {
  ...
  const d = new Date(`${String(date).slice(0, 10)}T${safeHour}:00`);
