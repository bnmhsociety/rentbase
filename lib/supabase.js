...
export async function supabaseInsert(path, body, { service = false } = {}) {
  const key = service ? getServerKey() : SUPABASE_ANON_KEY;
...
export async function supabasePatch(path, body, { service = false } = {}) {
  const key = service ? getServerKey() : SUPABASE_ANON_KEY;
...
export async function supabaseUpload(bucket, path, file, contentType = "application/octet-stream") {
  const key = getServerKey();
  ...
  const text = await response.text();
  if (!response.ok) {
    throw new Error(text || "Erreur upload Supabase Storage");
  }
