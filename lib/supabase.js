...
export function mediaProxyUrl(bucket, path) {
  ...
  return `/api/media?bucket=${encodeURIComponent(bucket)}&path=${encodeURIComponent(cleanPath)}&v=${version}`;
}

export async function signedUrl(bucket, path, expiresIn = 604800) {
  if (!path) return null;
